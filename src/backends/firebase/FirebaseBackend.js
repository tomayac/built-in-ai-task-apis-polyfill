...
`;
  ...
  /**
   * Checks availability of the rewriter model.
   * @param {object} options - RewriterCreateCoreOptions (ignored by this backend)
   * @returns {Promise<"available">}
   */
  ...
  /**
   * Helper to build the rewriter prompt from config.
   * @param {string} input
   * @param {object} config
   * @returns {string}
   */
  #buildRewriterPrompt(input, config) {
    let prompt = `You are a writing assistant.
  Follow these instructions precisely:
  ...
  /**
   * Performs non-streaming rewriting.
   * @param {string} input
   * @param {object} config
   * @returns {Promise<string>}
   */
  async rewrite(input, config) {
    const prompt = this.#buildRewriterPrompt(input, config);
    const signal = config.signal;
    try {
      const result = await this.#model.generateContent(prompt, { signal });
      return result.response.text();
  ...
  /**
   * Performs streaming rewriting.
   * @param {string} input
   * @param {object} config
   * @returns {ReadableStream<string>}
   */
  rewriteStreaming(input, config) {
    const prompt = this.#buildRewriterPrompt(input, config);
    const userSignal = config.signal;
  ...
  /**
   * Measures token usage for rewriting.
   * @param {string} input
   * @param {object} config
   * @returns {Promise<number>}
   */
  async measureRewriterUsage(input, config) {
    const prompt = this.#buildRewriterPrompt(input, config);
    const signal = config.signal;
    try {
      const result = await this.#model.countTokens(prompt, { signal });
